...
created_at: 2026-08-07T16:22:22+00:00
updated_at: 2026-08-07T16:43:03.933023+00:00
parent: STORE-I-0003
...
- [x] An initiative (or backlog item, at that repo's discretion) exists in
      ...
      constraint.
- [x] The scope is stated accurately: `sparql/memstore` is **299 lines of code and 2,415
      ...
      code is deleted.
- [x] The proposal states what does **not** change: the backend-independent core /
...
- [x] The sequencing constraint is explicit: odin-rdf-sparql must land its port **before**
...
- [x] Recorded in the proposal: **odin-rdf-sparql has no `bench/`**, so unlike
      odin-rdf-shacl there is no benchmark port on this side and no baseline to retire.
- [x] Test-count parity before and after, at both `Term_ID` widths, is an exit criterion
...
- **2026-08-07 — Filed as odin-rdf-sparql SPARQL-T-0023** (`#phase/backlog`), committed
  there as `99ab9e9`. Filed as a backlog item rather than an initiative, matching the
  family's established pattern for cross-repo requests — STORE-T-0019 and STORE-T-0022 both
  arrived here that way. It is a request; that repo promotes and sequences it.
  Carried across: the motivating decision (STORE-A-0006), the footprint (299 code / 2,415
  test lines across `eval.odin` and five test files), the no-assertion-rewriting rule, the
  absence of a `bench/` on that side, and the blocking relationship with STORE-T-0030 as the
  one part that is not their call. Also carried, beyond this task's original criteria: the
  **vision retraction** — their success criterion "in-memory and LMDB behave identically
  apart from performance" is falsified — plus the reassurance that their ordered-iteration
  groundwork survives, since kvstore's numeric-ID order stands on the big-endian key rule
  alone (STORE-A-0001 point 7, annotated in STORE-T-0025).